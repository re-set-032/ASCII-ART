...
class ASCII_ART:
    def __init__(self,file_path):
        ...
        # ASCII-charset of size 70 ranked based on intensity/lightness (high to low) (source => http://paulbourke.net/dataformats/asciiart/)
    ...
    def makeArt(self,tileWidth,tileHeight,use70):
        ...
        for i in range(0,h): 
            ...
            for j in range(0,w): 
                ...
                # select character based on avg intensity (one to many mapping) 
                if use70: 
                    gsval = self.ascii_70_charset[int((avg*69)/255)] 
                else: 
                    gsval = self.ascii_10_charset[int((avg*9)/255)] 
...
def main():
    ...
    parser.add_argument('--use70',dest='use70',action='store_true')      # by default uses 10 scale ascii characters
    ...
    # process image 
    ART.makeArt(tileWidth,tileHeight,args.use70)

    # write to text file (<= open this file to see output)
